...
def eval_policy(policy, eval_episodes=10, current_timestep=0, log_writer=None):
...
if __name__ == "__main__":
    ...
    file_name = f"{args.policy}_Secure_non_norm_{args.seed}"
    writer = SummaryWriter(log_dir=f"runs/{file_name}")
    print("---------------------------------------")
    print(f"Policy: {args.policy}, Env: Secure, Seed: {args.seed}")
    print("---------------------------------------")
    ...
    evaluations = [eval_policy(policy, current_timestep=0, log_writer=writer)]
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if (t + 1) % args.eval_freq == 0:
            evaluations.append(eval_policy(policy, current_timestep=t, log_writer=writer))
