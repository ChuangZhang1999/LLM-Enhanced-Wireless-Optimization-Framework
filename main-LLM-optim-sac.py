...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_timesteps", default=1e4, type=int)  # OpenAI gym environment name
    ...
    code_dir = os.path.join(args.dir, f'sac-run-v{args.v}-' + args.env)
    ...
    for train in range(args.sample_count):
        for seed_try in range(args.train_seed_count):
            cur_version = f'v{args.v}-{args.env}-train{train}_sac'
            cur_sid_result_path = f'{sid_result_dir}/train{train}_s{seed_try}_sac.npy'
            cur_seed = random.randint(0, 100000)
            cur_training_command = f'python LLM-optim-train-sac.py --env {args.env} --revise_path {revise_lib_path_buffer[train]} --version {cur_version} --sid_result_path {cur_sid_result_path} --seed {cur_seed} --max_timesteps {int(args.max_timesteps)} --intrinsic_w {args.intrinsic_w}'
