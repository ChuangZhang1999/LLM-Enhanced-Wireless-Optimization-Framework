...
class SAC(object):
    def __init__(self, state_dim, action_dim, max_action, gamma, tau, adaptive_alpha, batch_size, lr_a, lr_c, hidden_width):
        self.max_action = max_action
        ...
        if self.adaptive_alpha:
            ...
            self.alpha_optimizer = torch.optim.Adam([self.log_alpha], lr=lr_a)
        else:
            self.alpha = 0.2
    ...
    def learn(self, relay_buffer):
        batch_s, batch_a, batch_s_, batch_r, batch_dw = relay_buffer.sample(self.batch_size)  # Sample a batch

        with torch.no_grad():
            batch_a_, log_pi_ = self.actor(batch_s_)  # a' from the current policy
            # Compute target Q
            target_Q1, target_Q2 = self.critic_target(batch_s_, batch_a_)
            target_Q = batch_r + self.GAMMA * batch_dw * (torch.min(target_Q1, target_Q2) - self.alpha * log_pi_)
